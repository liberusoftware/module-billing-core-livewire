...
final class TaxCalculator extends Component
{
    ...
    public function calculate(CalculateTax $calculate): void
    {
        ...
        $this->validate(['amount' => ['required', 'numeric', 'min:0'], 'jurisdiction' => ['nullable', 'string', 'max:100']]);
        $teamId = data_get(auth()->user(), 'current_team_id') ?? data_get(auth()->user(), 'currentTeam.id');
        abort_if($teamId === null, 403, 'A current team is required.');
        $this->result = $calculate->execute((int) $teamId, (float) $this->amount, $this->jurisdiction ?: null);
    }
